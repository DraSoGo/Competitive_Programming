...
void geninput(string in, int c)
{
    ...
    if (c <= 15)
    {
        ...
        if (c == 0) {
            ...
            while (v1 == v2) v2 = valDist1(gen);
            for (int i = 0; i < n; i++) b.push_back((i % 2 == 0) ? v1 : v2);
        } else if (c == 4) {
            // N = 100 ไม่ซ้ำกันเลย
            n = 100;
    ...
    else
    {
        ...
        if (c == 16) {
            ...
            for (int i = 0; i < pool_size; i++) pool[i] = valDist2(gen);
            uniform_int_distribution<int> idxDist(0, pool_size - 1);
            
            for (int i = 0; i < n; i++) b.push_back(pool[idxDist(gen)]);
        } else {
            // N = 4000 สุ่มแบบเต็มข้อไร้ขีดจำกัด (Ultimate Stress Test)
            n = 4000;
            for (int i = 0; i < n; i++) b.push_back(valDist2(gen));
        }
    }
